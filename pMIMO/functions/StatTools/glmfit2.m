...
%Modified glmfit to save extra stat information:
%   (stats.max_iter, stats.bad_scaling_warned,
%   stats.ill_conditioned_warned, stats.iter_reached)
...
ill_conditioned_warned = false;
if rankx < ncolx
    if rankwarn
        warning(message('stats:glmfit:IllConditioned'));
        ill_conditioned_warned = true;
    end
    perm = perm(1:rankx);
    x = x(:,perm);
else
...
if nargout > 2
    ...
    %% Add additional ouput information!
    stats.max_iter = iter;
    stats.bad_scaling_warned = warned;
    stats.ill_conditioned_warned=ill_conditioned_warned;
    stats.iter_reached=iter_reached;
end
